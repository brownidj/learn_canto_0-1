...
class AssetDataRepository {
  ...
  Future<List<String>> loadCategories() async {
    ...
    final cats = await compute(_parseCategoriesFromYaml, text);
    cats.add('unassigned');
    final out = cats.toList()..sort();
    return out;
  }

  Future<Map<String, List<List<dynamic>>>> loadReverseIndex() async {
    final textJyut = await rootBundle.loadString('assets/data/reverse_jyut.yaml');
    final textManual = await rootBundle.loadString('assets/data/reverse_manual.yaml');
    final map1 = await compute(
      _parseReverseIndex,
      {'text': textJyut, 'defaultSource': 'tier1'},
    );
    final map2 = await compute(
      _parseReverseIndex,
      {'text': textManual, 'defaultSource': 'reverse_manual'},
    );
    final out = <String, List<List<dynamic>>>{};
    for (final entry in map1.entries) {
      out.putIfAbsent(entry.key, () => <List<dynamic>>[]).addAll(entry.value);
    }
  ...
  Future<HkWordsData> loadHkWords() async {
    ...
    final parsed = await compute(_parseHkWords, text);
    return HkWordsData(
      freqMap: Map<String, double>.from(parsed['freqMap'] as Map),
      colloquial: Set<String>.from(parsed['colloquial'] as List),
      attested: Set<String>.from(parsed['attested'] as List),
    );
  }
  ...
  Future<Map<String, List<String>>> loadCccantoMeanings() async {
    ...
    final out = await compute(_parseCccanto, text);
    _cccantoCache = out;
    return out;
  }
  ...
  Future<Map<String, List<String>>> loadCedictMeanings() async {
    ...
    final out = await compute(_parseCedict, text);
    _cedictCache = out;
    return out;
  }
  ...
  Future<Map<String, dynamic>> loadLegacyVocab() async {
    final local = await _loadLocalVocab();
    final baseText = local ?? await rootBundle.loadString('assets/data/vocab.yaml');
    final parsed = await compute(_parseLegacyVocab, baseText);
    return parsed;
  }
...
Set<String> _parseCategoriesFromYaml(String text) {
...
Map<String, List<List<dynamic>>> _parseReverseIndex(Map<String, dynamic> args) {
  final text = args['text'] as String? ?? '';
  final defaultSource = args['defaultSource'] as String? ?? 'tier1';
  final out = <String, List<List<dynamic>>>{};
  final doc = loadYaml(text);
  if (doc is! YamlMap) {
    return out;
...
Map<String, dynamic> _parseHkWords(String text) {
  final rows = const CsvToListConverter(eol: '\n').convert(text, shouldParseNumbers: false);
  if (rows.isEmpty) {
    return {'freqMap': <String, double>{}, 'colloquial': <String>[], 'attested': <String>[]};
  }
  ...
  return {
    'freqMap': freqMap,
    'colloquial': colloq.toList(),
    'attested': attested.toList(),
  };
}

Map<String, List<String>> _parseCccanto(String text) {
...
Map<String, List<String>> _parseCedict(String text) {
  final out = <String, List<String>>{};
...
Map<String, dynamic> _parseLegacyVocab(String baseText) {
  final obj = _yamlToObject(loadYaml(baseText));
  final data = obj is Map<String, dynamic> ? obj : <String, dynamic>{};
